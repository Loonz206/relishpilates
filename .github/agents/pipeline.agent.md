...
## Phase 1 — Research _(skip if no external library or API involved)_
...
- Run `pnpm lint` (up to 3 attempts)
...
## Phase 5 — Unit Testing _(skip if Jest not installed)_
...
- Run `pnpm test` and repair failures (up to 3 attempts)
- Verify the 80% coverage gate via `pnpm test --ci --coverage`

## Phase 6 — E2E Testing _(skip if Playwright not installed)_
...
## Phase 7 — Docs Sync + Reflect _(run in parallel)_
