...
pnpm add -D jest jest-environment-jsdom @testing-library/react @testing-library/jest-dom @types/jest ts-jest ts-node
```
...
- The threshold is enforced in `jest.config.ts` — `pnpm test:coverage` will fail below 80%
...
3. Run a focused suite first:
   - Single file: `pnpm test -- --testPathPattern="path/to/Component"`
...
5. If tests fail:
   ...
   d. Re-run focused scope, then re-run `pnpm test`.
   e. Retry up to **3 attempts total**.
6. If failures persist after 3 attempts → record remaining failures and stop.
7. Once all tests pass, run `pnpm test:coverage` to verify the coverage gate.
8. If coverage drops below 80% on any metric:
   ...
   c. Re-run `pnpm test:coverage`. Retry up to 3 attempts.

## Constraints

- Do not run `pnpm lint`, `npx tsc`, or Playwright.
